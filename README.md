...
- **一键初始化** — 自动配置审计策略、开启 DNS 日志、下载安装 Sysmon、注册定时任务、启动 ETW 流量会话
- **多源数据采集** — 从 7 个数据源采集网络活动：6 个 Windows Event Log/Sysmon 源（Security 5156/4688、DNS 3008、Sysmon 1/3/11）+ 1 个 ETW 实时会话（Kernel-Network Send/Recv，按进程统计字节）
- **每进程流量归因** — 真正回答"哪个应用在吃流量"：基于 ETW Kernel-Network 字节级事件，按进程聚合上传/下载字节，与连接次数独立统计
- **智能异常检测** — 自动识别：非常用端口连接、凌晨异常活动、DNS 解析失败（疑似 DGA）、首次出现域名、大文件下载、可疑进程
- **结构化日报** — 中英双语，统计汇总 + 明细记录，记事本/VS Code 可直接打开，方便 AI 分析
- **零外部依赖** — 纯 PowerShell，Sysmon 自动下载，ETW 会话用 Windows 内置 `logman`，无需安装任何第三方工具
- **自动维护** — 定时采集（每日 00:30 + 开机自启）、ETW 会话开机自恢复、过期日志自动清理
